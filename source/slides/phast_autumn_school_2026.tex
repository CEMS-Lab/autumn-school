...
\begin{frame}{One specimen. Three learning questions.}
\vspace{4mm}
\begin{tabular}{@{}L{.10\textwidth}L{.82\textwidth}@{}}
\color{teal}\Large A & \Large Why does a crack grow?\\[7mm]
\color{teal}\Large B & \Large How do we compute and differentiate it?\\[7mm]
\color{teal}\Large C & \Large When can a learned proposal be useful?\\
\end{tabular}
\vspace{6mm}
...
\flowfour{Mechanics\\$u^{k+1}$}{Driving field\\$\mathcal H^{k+1}$}{Damage\\$d^{k+1}$}{Check\\convergence}
\vspace{4mm}
\begin{center}\begin{tikzpicture}[>=Latex]
\draw[->,teal,thick] (11,0)--(11,-.55)--(0,-.55)--(0,0);
\node[muted] at (5.5,-1.0) {repeat within one load/time increment};
...
\vspace{4mm}
\flowfour{Gather\\local values}{Apply\\local physics}{Accumulate\\global result}{Iterate\\with checks}
...
\flowfour{Parameter\\$\theta$}{Supported\\solve}{Observation\\$y(\theta)$}{Scalar loss\\$\mathcal L$}
\vspace{3mm}
\begin{center}\begin{tikzpicture}[>=Latex]
\draw[<-,teal,thick] (0,0)--(11,0);
\node[teal] at (5.5,-.6) {backward: $\partial\mathcal L/\partial\theta$};
\end{tikzpicture}\end{center}
...
\timing{B 78--85 / 7 minutes}{Explain backpropagation as chain-rule accumulation through a supported graph. PyTorch and JAX are frameworks; PhAST and JAX-FEM are solver projects. Both ecosystems can support differentiable FEM. Framework choice alone is not a gradient or performance guarantee.}
\end{frame}
...
\begin{center}\begin{tikzpicture}[>=Latex,every node/.style={align=center},thick]
\node (p) at (0,0) {assessed\\proposal};
\node[teal] (a) at (7,1.1) {accept\\record and advance};
\node[navy] (r) at (7,-1.1) {reference correction\\record why};
\draw[->,teal] (p)--node[above,sloped,font=\small]{passes stated gate}(a);
\draw[->,navy] (p)--node[below,sloped,font=\small]{fails stated gate}(r);
\end{tikzpicture}\end{center}
...
\flowfour{Visit\\model states}{Query\\reference}{Aggregate\\labelled data}{Retrain\\and reassess}
\vspace{4mm}
\begin{center}\begin{tikzpicture}[>=Latex]
\draw[->,teal,thick] (11,0)--(11,-.6)--(0,-.6)--(0,0);
\node[muted] at (5.5,-1.05) {fixed held-out evaluation; reference queries have a cost};
\end{tikzpicture}\end{center}
