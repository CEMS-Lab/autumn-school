...
Fixed mesh connectivity, logging and parameter-independent preprocessing
remain outside that differentiated path. A custom implicit backward rule can cover
...
analytic profile normal to a crack. Increasing $\ell_0$ changes the
represented damage width and the model. Smoothness of the
parameter-to-observation map is a separate property.

**A kink in an update.** $\max(a,b)$ is continuous: its value approaches
the tie continuously, while its slope changes abruptly. A step function
...
evolution. The local derivative describes the current numerical trajectory;
alternative branches require their own forward evaluations.
...
and chaotic long-time sensitivity. Compare the smoothness and conditioning
of the particular discrete maps used in each problem.
...
Here $\psi^+$ is the configured tensile driving energy density, evaluated
from the strain state. On unloading the current energy can fall, while $H$ remembers
the earlier maximum. PhAST combines this history update with explicit
damage bounds in the complete constrained algorithm.
...
operand at equality. This is the selected differentiation convention at the
kink. If both operands have identical parameter derivatives, their
...
identified by its effective configuration and the operator that it selects.

The configuration names are `hard_max` for the hard maximum, `softmax` for
log-sum-exp, and `custom_subgrad` for the custom reverse rule. The scaled
research variant is named `custom_subgrad_dimensionless`.

| History choice | Forward value | Reverse rule | Meaning |
|---|---|---|---|
| Hard maximum | $\max(a,b)$ | Active branch; half at a tie | Exact local rule away from switches |
| Log-sum-exp | $S_\beta(a,b)$ | Smooth-forward derivative | Changes the history model |
| Custom reverse | $\max(a,b)$ | Sigmoid weights | Surrogate near a switch |
| Scaled custom reverse | $\max(a,b)$ | Scaled sigmoid weights | Surrogate with explicit energy units |
...
must be recorded to identify the route used by each experiment.
...
Numerically use `logsumexp`, `logaddexp` or stable softplus to evaluate the
exponential terms stably. Its one-update bias satisfies
...
therefore occur at constant loading. Assess its accumulated effect over the
complete trajectory as well as its local magnitude.
...
$\psi_{\rm ref}$ is a fixed positive reference energy, held constant during
differentiation. Converting energy units requires converting that
reference too. Very sharp sigmoids can saturate to zero or one in finite
precision, producing a zero weight on one branch.
...
are dimensionless teaching examples of the history operators.
```
...
measured residuals to establish the achieved solve accuracy.
...
| Gradient saturation or uninformative loading | Observation sensitivity and individual gradient routes | A correct zero derivative can be physical; consider richer observations and loading |
...
| Long trajectories or unstable time steps | Gradient norm versus horizon, stability and replay agreement | Products can vanish or amplify; checkpointing addresses memory; stability needs a separate analysis |
...
| Non-identifiable parameters | Observation-Jacobian rank and alternate parameter fits | Assess uniqueness using independent data and alternate fits alongside derivative checks |
...
range of $h$. Per-node active-set records locate branch changes. Aggregate
counts provide a coarser diagnostic: differing counts imply changed memberships,
while equal counts permit different memberships. A Taylor remainder with
quadratic decay supports the derivative on the tested smooth, resolved interval.

These operational checks apply at specified initializations and operating
points. Recovery depends on loading,
...
earlier loading. The total parameter derivative includes the stored-history
route as well as the current-energy route.
:::
...
the two. The unequal one-sided slopes identify the kink even when the
software convention matches this average.
:::
...
iterates can therefore differ. Assess recovery for each selected model and
reverse rule.
:::
